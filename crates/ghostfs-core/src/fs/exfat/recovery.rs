...
impl<'a> ExFatRecoveryEngine<'a> {
    ...
    pub fn scan_deleted_files(&self) -> Result<Vec<DeletedFile>> {
        ...
        tracing::info!("exFAT Recovery complete: {} total files found", deleted_files.len());
        Ok(deleted_files)
    }
                ...
                Ok(DirectoryEntry::File(file_entry)) => {
                    ...
                    if let Some(file_set) = FileEntrySet::parse_from_entries(&entries) {
                        ...
                        if file_set.stream_extension.first_cluster >= 2 {
                            let file_chain = self.fat_table.get_chain(
                                file_set.stream_extension.first_cluster
                            );
                            for c in &file_chain {
                        ...
                        if file_set.is_deleted {
                            deleted_files.push(self.file_set_to_deleted_file(
                                *file_id,
                                &file_set,
                            ));
                            *file_id += 1;
        ...
        for cluster in 2..max_clusters + 2 {
            ...
            if let Ok(header) = self.device.read_bytes(offset, 512) {
                if let Some((mime, ext, est_size)) = self.detect_file_with_size(&header, offset) {
                    let block_count = (est_size + cluster_size - 1) / cluster_size;
    ...
    fn file_set_to_deleted_file(&self, id: u64, file_set: &FileEntrySet) -> DeletedFile {
        ...
        tracing::debug!(
            ...
            file_set.filename, first_cluster, chain.len(), file_set.stream_extension.data_length
        );
    ...
    fn chain_to_block_ranges(&self, chain: &[u32]) -> Vec<BlockRange> {
        ...
        tracing::debug!(
            ...
            chain, cluster_size, first_offset
        );
    ...
    fn detect_file_with_size(&self, header: &[u8], start_offset: u64) -> Option<(String, String, u64)> {
        ...
        if header.starts_with(&[0x50, 0x4B, 0x03, 0x04]) {
            let size = std::cmp::min(
                self.boot_sector.volume_length * self.boot_sector.bytes_per_sector() as u64 - start_offset,
                1024 * 1024
            );
...
mod tests {
    ...
    fn test_file_type_detection() {
        ...
        assert_eq!(engine_detect(&[0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46]).0, "image/jpeg");
        assert_eq!(engine_detect(&[0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]).0, "image/png");
    }
}
